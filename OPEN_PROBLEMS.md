...
**Update — April 2026:** The TETRA technology review (Kyle Mickey, Corewood, April 2026) identifies TETRAFILE as a serious candidate substrate for the artifact layer of this problem. TETRAFILE's multi-recipient cryptographic addressability allows a governance decision formed in one domain to be made readable by a receiving domain through a recipient addition rather than re-derivation. The formation event and the portable artifact are the same object. The receiving domain verifies the signature and has the commitment as a single self-validating unit without reopening the prior act. The reverse-engineering resistance of the format reinforces this: the receiving domain is not evaluating an inspectable payload with optional verification — it is evaluating an artifact whose only legitimate interpretation path is the signature-verified read through the authorised reader.

TETRAFILE is the strongest candidate substrate identified to date for the artifact layer. However the standing question remains open. Whether the artifact is no longer referential to antecedent authority — whether the object is non-referential in the strong sense, such that antecedent authority has ceased to exist as a live operative basis — is the decisive test that has not yet been satisfied. A self-validating artifact is not the same thing as an object with ontological finality. The gap between these two things is the remaining open question on this problem.

This does not resolve the regulatory recognition problem either. A technical primitive for a portable act does not by itself make that act a primary object of reliance under DUAA or EU AI Act. Both the standing question and the regulatory recognition question remain open. The specification records TETRAFILE as the strongest available candidate without claiming either question is closed.
