...
**Update — June 2026:** LGT.io's reference implementation (truc) addresses the artifact layer of this problem through multi-recipient cryptographic addressability in the governed artifact format. A governance decision formed in one domain can be made readable by a receiving domain through a recipient addition rather than re-derivation — the formation event and the portable artifact are the same object. The receiving domain verifies the signature and has the commitment as a single self-validating unit without reopening the prior act. The post-quantum cryptographic construction of the artifact format provides forward-security over the commitment lifetime. Architecture decision records are published at github.com/lgt-io/truc.

This does not resolve the regulatory recognition problem. A technical primitive for a portable act does not by itself make that act a primary object of reliance under DUAA or EU AI Act. The standing question and the regulatory recognition question remain open. The truc artifact format is the strongest available candidate for the technical layer without claiming either question is closed.

**Further refinement — the antecedent authority test (Peter Cranstone, April 2026):**

Subsequent discussion has sharpened the problem to a more precise formulation. The technical properties of a well-constructed governed artifact make it the strongest available carrier of a governance commitment, but the question of whether it constitutes a portable act rather than portable evidence depends on a test that is prior to technical capability.
...
This is a tractable policy objective. The precedents show it has been achieved before. The technical primitive exists in the reference implementation. The open question is which policy body, regulatory framework, or international agreement creates the normative designation.

**What this means for practitioners:**

For a team deploying AI agents in UK mortgage lending, or a wealth management firm deploying agents across an adviser network operating under consistent FCA oversight, the antecedent authority question does not arise in practice. All participants share the governance context. The architecture as specified — confidence-weighted authorisation, human-resolution events at appropriate thresholds, cryptographically-sealed governed artifact — is sufficient to produce decisions that hold across the deployment.
...
**Update — June 2026:** The LGT.io reference implementation (truc) uses post-quantum cryptographic primitives drawn from current NIST FIPS post-quantum standards (ML-KEM, ML-DSA) in hybrid mode, rather than classical public-key primitives. This removes the harvest-now-decrypt-later exposure that classical primitives face over long commitment lifetimes — artifacts produced today are not subject to decryption by quantum adversaries over the lifetime of commitments recorded in them.

This should be understood as solving a substrate dependency rather than the problem itself. It addresses cryptographic forward-security over long time horizons — an important prerequisite. It does not address the temporal validity problem as formulated in this specification, which concerns regulatory drift, trigger conditions for re-evaluation, validator mesh behaviour under changed guidance, and whether previously lawful commitments remain actionable when the surrounding legal context moves. Those questions remain open. The post-quantum substrate is a necessary condition for long-lived commitment integrity, not a sufficient one.
